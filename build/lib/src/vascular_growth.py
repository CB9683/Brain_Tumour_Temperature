...
def initialize_perfused_territory_and_terminals(
    ...
    if initial_graph:
        for node_id, data in gbo_graph.nodes(data=True): # Iterate over gbo_graph (the copy)
    ...
    if initial_graph and initial_graph.number_of_nodes() > 0:
        for node_id, data in initial_graph.nodes(data=True): # Iterate over original to find terminals
            if data.get('type') == 'measured_terminal':
                ...
                node_attrs_sprout['Q_flow'] = term_gbo_data.flow # Ensure Q_flow is set from GBOIterationData
                node_attrs_sprout.pop('current_territory_voxel_indices_flat', None)
                node_attrs_sprout.pop('current_territory_demand', None)
                # Single call to add/update node with all correct attributes
                data_structures.add_node_to_graph(gbo_graph, term_gbo_data.id, **node_attrs_sprout)
                # No second call to data_structures.add_node_to_graph needed
    ...
    if not processed_from_initial_graph:
        ...
        if seed_points_config and isinstance(seed_points_config, list):
            ...
            for seed_info in seed_points_config:
                ...
                node_attributes_seed['type'] = 'synthetic_root_terminal'
                node_attributes_seed['Q_flow'] = term_gbo_data.flow # Ensure Q_flow is set from GBOIterationData
    ...
    if not active_terminals: # Fallback seed
        ...
        if tissue_data.get('domain_mask') is not None and np.any(tissue_data['domain_mask']):
            ...
            if valid_seed_points_vox.shape[0] > 0:
                ...
                node_attrs_fallback['Q_flow'] = term_gbo_data.flow
...
def grow_healthy_vasculature(config: dict,
    ...
    for iteration in range(max_iterations):
        ...
        for term_p_gbo_data in terminals_for_growth_attempt:
            ...
            if attempt_branching and len(frontier_voxels_global_flat_indices) > 0:
                ...
                if len(combined_territory_indices) < 2:
                    ...
                    if bifurcation_result:
                        ...
                        # Preserve is_flow_root if parent was a root
                        parent_is_flow_root = gbo_graph.nodes[term_p_gbo_data.id].get('is_flow_root', False)
                        gbo_graph.nodes[term_p_gbo_data.id].update(
                            type='synthetic_bifurcation',
                            radius=new_parent_radius,
                            Q_flow=new_parent_total_flow,
                            is_flow_root=parent_is_flow_root # Explicitly carry over
                        )
                        logger.debug(f"Bifurcating node {term_p_gbo_data.id}: is_flow_root set to {parent_is_flow_root}")
                        ...
                        for i_child, (child_pos, child_rad, child_flow) in enumerate([(c1_pos, c1_rad, c1_total_flow), (c2_pos, c2_rad, c2_total_flow)]):
                            ...
                            child_node_attrs = vars(child_gbo).copy()
                            child_node_attrs['type'] = 'synthetic_terminal'
                            child_node_attrs['Q_flow'] = child_gbo.flow # Ensure Q_flow from GBOIterationData
                            # is_flow_root for children is False (default, not present in attrs)
                            child_node_attrs.pop('current_territory_voxel_indices_flat', None)
                            child_node_attrs.pop('current_territory_demand', None)
                            data_structures.add_node_to_graph(gbo_graph, child_gbo.id, **child_node_attrs)
                            # No second call to data_structures.add_node_to_graph

                            data_structures.add_edge_to_graph(gbo_graph, term_p_gbo_data.id, child_gbo.id,
                                                              length=child_gbo.length_from_parent,
                                                              radius=new_parent_radius, # Edge takes parent's new radius
            ...
            if not attempt_branching:
                ...
                if len(frontier_voxels_global_flat_indices) > 0:
                    ...
                    all_coords_for_centroid_list = [newly_acquired_coords]
                        ...
                        if extension_length > constants.EPSILON:
                            ...
                            if gbo_graph.has_node(term_p_gbo_data.id) and term_p_gbo_data.parent_id and \
                               gbo_graph.has_edge(term_p_gbo_data.parent_id, term_p_gbo_data.id):
                                ...
                                if gbo_graph.has_node(term_p_gbo_data.parent_id): # Ensure parent exists
        ...
        if current_active_terminals and np.any(perfused_tissue_mask):
            ...
            if live_terminals_for_adaptation:
                ...
                if perfused_global_flat_indices.shape[0] > 0 :
                    ...
                    for t_data in live_terminals_for_adaptation:
                ...
                if run_flow_solver_this_iteration:
                    ...
                    for term_obj in live_terminals_for_adaptation:
                        if gbo_graph.has_node(term_obj.id): gbo_graph.nodes[term_obj.id]['Q_flow'] = term_obj.flow
                    
                    # For debugging: Check is_flow_root status before calling solver
                    num_flow_roots_in_graph = 0
                    for node_id_debug, node_data_debug in gbo_graph.nodes(data=True):
                        if node_data_debug.get('is_flow_root', False):
                            num_flow_roots_in_graph +=1
                            logger.debug(f"Flow Solver PRE-CALL: Node {node_id_debug} is_flow_root=True, Type: {node_data_debug.get('type')}")
                    if num_flow_roots_in_graph == 0:
                        logger.error("Flow Solver PRE-CALL: CRITICAL - No flow roots detected in the graph before calling solver!")
                    else:
                        logger.info(f"Flow Solver PRE-CALL: Found {num_flow_roots_in_graph} flow root(s) in the graph.")
                            ...
                            if node_type_for_adapt == 'synthetic_terminal':
                                ...
                                for u_edge_in, _, edge_data_in in gbo_graph.in_edges(node_id_adapt, data=True):
                                    ...
                                    if solved_in_flow is not None and np.isfinite(solved_in_flow):
                                        ...
                                        break
                            elif node_type_for_adapt == 'synthetic_bifurcation' or node_type_for_adapt == 'synthetic_root_terminal': # Roots that bifurcated
                                ...
                                logger.info(f"MurrayDebug - Bif/RootTerm {node_id_adapt}: Children solved flows sum to actual_node_flow={actual_node_flow:.3e}. Original R={original_radius_before_adapt:.4f}. New R will be {potential_new_r_debug:.4f}")
                            else:
                                continue

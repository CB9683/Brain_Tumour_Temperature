...
def grow_healthy_vasculature(config: dict,
    ...
    for iteration in range(max_iterations):
                    ...
                    if gbo_graph_with_flow:
                        ...
                        for node_id_adapt in nodes_to_adapt_gbo:
                            ...
                            if is_sink_node:
                                for term_obj_sync in live_terminals_for_adaptation:
                                    if term_obj_sync.id == node_id_adapt:
                                        ...
                                        break
                            if node_data_adapt.get('is_flow_root'): logger.debug(f"GlobalAdapt GBO: Node {node_id_adapt} is flow_root. Radius not adapted."); continue
                            if abs(actual_node_flow) > constants.EPSILON:
                                ...
                                if not np.isclose(original_radius_before_adapt, new_radius_adapted, rtol=1e-2, atol=1e-5): logger.info(f"GlobalAdapt GBO: Node {node_id_adapt} ({node_type_for_adapt}) R: {original_radius_before_adapt:.4f} -> {new_radius_adapted:.4f}")
                                node_data_adapt['radius'] = new_radius_adapted
                            elif original_radius_before_adapt > min_radius + constants.EPSILON :
                                logger.info(f"GlobalAdapt GBO: Node {node_id_adapt} ({node_type_for_adapt}) Q_sum near zero. Shrinking R to {min_radius:.4f}.")
                        ...
                        logger.info("Global radius adaptation complete.")
                    else: logger.error("Flow solver did not return graph. Skipping GBO radius adaptation.")
        ...
        # MODIFIED Blender Snapshot Logic
        if blender_snapshot_interval > 0 and (current_iteration_num % blender_snapshot_interval == 0 or iteration == max_iterations -1):
            ...
            if affine_matrix is not None:
                ...
                # Save overall domain mask for context (optional: with every snapshot or only first)
                ...
                if current_iteration_num == blender_snapshot_interval or save_domain_with_each_snapshot or iteration == max_iterations -1:
                    # Choose which domain mask to save for context, e.g., gbo_growth_domain_mask or anatomical_domain_mask

...
class GBOIterationData:
    def __init__(self, terminal_id: str, pos: np.ndarray, radius: float, flow: float, 
        ...
        self.pos: np.ndarray = np.array(pos, dtype=float) # Ensure float for positions
...
def initialize_perfused_territory_and_terminals(
    ...
    if not processed_from_initial_graph:
        ...
        if seed_points_config and isinstance(seed_points_config, list):
            logger.info(f"Using {len(seed_points_config)} seed points from configuration.")
            for seed_info in seed_points_config:
                ...
                next_synthetic_node_id += 1
            processed_from_initial_graph = True 
        elif seed_points_config: # If it exists but is not a list or empty
             logger.warning("`gbo_growth.seed_points` in config is not a valid list or is empty.")
    ...
    for term_data in active_terminals:
        ...
        current_radius_before_omega_init_update = term_data.radius # Store to ensure it doesn't shrink below seed/min
        ...
        # If no demand, flow and radius remain as initialized (seed R/Q or sprout R/Q)
...
def grow_healthy_vasculature(config: dict,
    ...
    if not current_active_terminals: return gbo_graph 
    ...
    if total_voxels_in_domain == 0:
        logger.error("Domain mask is empty. Cannot proceed.")
        return gbo_graph
    ...
    for iteration in range(max_iterations):
        ...
        if unperfused_voxels_3d_indices.shape[0] > 0:
            ...
            if np.any(~valid_mask_kdt): # If some were invalid, filter coords for KDTree
        ...
        if kdtree_unperfused is None or kdtree_unperfused.n == 0:
            logger.info("GBO: No unperfused domain voxels for KDTree. Target likely achieved.")
            break
        ...
        for term_p_gbo_data in terminals_for_growth_attempt:
            ...
            if not attempt_branching: 
                ...
                if len(frontier_voxels_global_flat_indices) > 0:
                    ...
                    all_coords_for_centroid = [newly_acquired_coords]
                    if term_p_gbo_data.current_territory_voxel_indices_flat:
                        valid_curr_idx = [idx for idx in term_p_gbo_data.current_territory_voxel_indices_flat if idx < tissue_data['world_coords_flat'].shape[0]]
                        if valid_curr_idx: all_coords_for_centroid.insert(0, tissue_data['world_coords_flat'][valid_curr_idx])
                    
                    if all_coords_for_centroid:
                        all_supplied_coords = np.vstack(all_coords_for_centroid)
        ...
        if current_active_terminals and np.any(perfused_tissue_mask):
            ...
            if live_terminals_for_voronoi:
                ...
                if perfused_global_flat_indices.shape[0] > 0 :
                    ...
                    term_flows_vor = np.array([t.flow if t.flow > constants.EPSILON else default_initial_flow for t in live_terminals_for_voronoi])
                    ...
                    for t_data in live_terminals_for_voronoi:
                        ...
                        if gbo_graph.has_node(t_data.id):
                            gbo_graph.nodes[t_data.id].update(Q_flow=t_data.flow, radius=t_data.radius)
